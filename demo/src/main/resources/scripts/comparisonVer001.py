...
# 파일 경로 : controller 단에서 저장하고 사용할거임(db는 안쓰고 DTO에다가만 저장)
# 주소를 string 형식으로 저장할거니까 나중에 보내기만 하면 됨
...
# 아니면 기준이 되는 컬럼의 좌표를 입력하게 한뒤 그것을 기준으로 삼을까?

# 여기서 필요한 결과 : a, b의 기준값, 비교대상이 되는 컬럼 가져오기
# 컬럼 주소와 그 안의 이름을 가져올 것.
# 시트가 여러 개 있다면
# 시트001 : 컬럼(A,1), 시트001 : 컬럼(C,1).. 등
# A의 기준 컬럼 값, B의 
